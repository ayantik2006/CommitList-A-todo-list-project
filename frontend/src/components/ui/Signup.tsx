...
function Signup() {
  ...
  return (
    <div className="w-screen h-screen bg-black flex flex-col justify-center items-center">
      <form
        ...
        onSubmit={(e) => {
          ...
          fetch("https://commitlist-backend.onrender.com/auth/signup", {
            ...
            .then((res) => res.json())
            .then((res) => {
              if (res.msg === "password error") {
                toast.error("Passwords are not matching!");
              } else if (res.msg === "email error 1") {
                toast.error("An user with this email already exists!");
              } else if (res.msg === "email error 2") {
                toast.info(
                  "Verification link has already been sent! Please try to resend after some time."
                );
      ...
      >
        {/* main box */}
        <h1 className="text-white text-3xl font-bold">Create an account</h1>
        ...
        <span className="relative top-[2rem]">
          <i className="fa-solid fa-lock text-gray-500 z-10 relative left-[6%] top-[40%]"></i>
          <input
            type={showPass2 ? "text" : "password"}
            placeholder="Confirm Password"
            name="confirmPassword"
            className="w-80 h-9 border-1 border-gray-500 rounded-md relative left-[-0.5%] text-white pl-7 top-4 bg-gray-950 pr-8"
            required
          />
          <button
            type="button"
            className="text-gray-500 relative left-[-8%] top-[0.9rem] hover:cursor-pointer"
            onClick={() => {
              setShowPass2(!showPass2);
            }}
          >
            <i
              className={`fa-solid ${showPass2 ? "fa-eye" : "fa-eye-slash"}`}
            ></i>
          </button>
        </span>
        <button
      ...
      <p className="text-gray-500 relative top-20">
        ...
        <a className="text-gray-400 cursor-pointer" onClick={()=>{
            navigate("/signin");
        }}> Sign in</a>
      </p>
    </div>
